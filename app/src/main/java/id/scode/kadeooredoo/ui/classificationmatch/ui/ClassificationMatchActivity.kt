 ...
 */
class ClassificationMatchActivity : AppCompatActivity() {
    ...
    private val mHidePart2Runnable = Runnable {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            txt_title_classificaton_match.systemUiVisibility =
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        txt_title_classificaton_match.setOnClickListener { toggle() }

        // Upon interacting with UI controls, delay any scheduled hide()
        // operations to prevent the jarring behavior of controls going away
        // while interacting with the UI.
        spinner_classification.setOnTouchListener(mDelayHideTouchListener)

        intent.also {
            idLeague = it.getStringExtra(DETAIL_KEY)
        }
        txt_title_classificaton_match?.text = idLeague
